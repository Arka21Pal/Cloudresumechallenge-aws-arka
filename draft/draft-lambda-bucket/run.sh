...
#     table_name="visitor-count"
    ...
    bucket="${stack_name}-website-bucket-unique"
    filename="url"
    code_file="${filename}.js"
    target="s3://${bucket}"

    template="lambda-bucket.yaml"
...
#         aws lambda update-function-configuration --function-name "${function_name}" --environment "Variables={table_name='${table_name}', aws_access_key_id='${aws_access_key_id}', aws_secret_access_key='${aws_secret_access_key}', region_name='${region_name}'}" --region "${region_name}" --profile "${profile}"
        aws lambda update-function-configuration --function-name "${function_name}" --environment "Variables={aws_access_key_id='${aws_access_key_id}', aws_secret_access_key='${aws_secret_access_key}', region_name='${region_name}'}" --region "${region_name}" --profile "${profile}"

        # Upload file to be written to to bucket
